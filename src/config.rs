...
impl Config {
    pub fn new(config_path: Option<&str>) -> Result<Self> {
        ...
        let db_name = env::var("MONGO_PERSONAL_PERFORMANCE_DB")?;
        let db_user_name = env::var("MONGO_PERSONAL_PERFORMANCE_USER")?;
        let db_user_password = env::var("MONGO_PERSONAL_PERFORMANCE_PASSWORD")?;
        let db_host = env::var("MONGO_PERSONAL_PERFORMANCE_HOST")?;
...
pub mod unit_tests_config {
    ...
    fn get_config() -> Result<()> {
        ...
        let test_cases = vec![
            TestCase {
                ...
                setup_env_file: Some(
                    r#"
                    MONGO_PERSONAL_PERFORMANCE_DB=wegonice
                    MONGO_PERSONAL_PERFORMANCE_USER=niceUser
                    MONGO_PERSONAL_PERFORMANCE_PASSWORD=nicePassword
                    MONGO_PERSONAL_PERFORMANCE_HOST=127.0.0.1:27017
        ...
        for t in test_cases {
            ...
            match t.expected_config {
                Some(expected_config) => {
                    ...
                    assert_eq!(config.db_name, expected_config.db_name, "{}", &t.title,);
                    assert_eq!(
                        config.db_user_name, expected_config.db_user_name,
                        "{}",
                        &t.title,
                    );
                    assert_eq!(
                        config.db_user_password, expected_config.db_user_password,
                        "{}",
                        &t.title
                    );
                    assert_eq!(
    ...
    fn clear_env_vars() -> Result<HashMap<String, String>> {
        ...
        let env_var_keys = vec![
            "MONGO_PERSONAL_PERFORMANCE_DB",
            "MONGO_PERSONAL_PERFORMANCE_USER",
            "MONGO_PERSONAL_PERFORMANCE_PASSWORD",
            "MONGO_PERSONAL_PERFORMANCE_HOST",
            "SERVER_HOST",
        ];
